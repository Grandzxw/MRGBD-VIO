...
class FeatureTracker
{
  public:
    ...
    void readImage(const cv::Mat &_img,const cv::Mat &_depth);
    ...
    cv::Mat prev_depth, cur_depth, forw_depth;  //当前的深度值与之前的深度值同上
    vector<cv::Point2f> n_pts;  //每一帧中新提取的特征点
    vector<cv::Point2f> prev_pts, cur_pts, forw_pts;  //每一帧的特征点个数
    vector<int> ids;                     //  每个特征点的id  能够被跟踪到的id
