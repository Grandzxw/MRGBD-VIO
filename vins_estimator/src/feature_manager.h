...
class FeaturePerId
{
  public:
    ...
    //  feature_per_frame 是个向量容器，存着这个特征在每一帧上的观测量。 存的是每个特征在每一帧上的观测量
    ...
    FeaturePerId(int _feature_id, int _start_frame, double _measure_depth)
        : feature_id(_feature_id), start_frame(_start_frame),
          used_num(0), estimated_depth(-1.0),measured_depth(_measure_depth), estimate_flag(0),solve_flag(0)
    {
...
class FeatureManager
{
  public:
    ...
    // mono point
//    bool addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Vector4d>>> &image);
    // mono line
    bool addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 4, 1>>>> &image, const map<int, vector<pair<int, Vector4d>>> &lines);
    // stereo line
    ...
    void removeOutlier();
  public:
    // 通过FeatureManager可以得到滑动窗口内所有的角点信息以及线特征的信息
