 ...
 * @param[in]   image 某帧所有特征点的 构成的map,索引为feature_id
 * @param[in]   header 某帧图像的头信息
 * @return  void
*/
//使用这个函数来进行处理图像
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 4, 1>>>> &image, const map<int, vector<pair<int, Vector4d>>> &lines, const std_msgs::Header &header)
{
    ROS_DEBUG("new image coming ------------------------------------------");
    ROS_DEBUG("Adding feature points %lu", image.size());
    //if (f_manager.addFeatureCheckParallax(frame_count, image))           // 当视差较大时，marg 老的关键帧  比较视差
    ...
    //imageFrame帧对齐
    //将图像数据、时间、临时预积分值存在图像帧类中  记录上一次的预积分值
    ...
    if (solver_flag == INITIAL)
    {
        ...
        {
            //frame_count是滑动窗口中图像帧的数量，一开始初始化为0，滑动窗口总帧数WINDOW_SIZE=10  滑动窗口总数
            bool result = false;
...
bool Estimator::initialStructure()
{
    TicToc t_sfm;
    //判断imu有无运动充分  通过加速度判断
    //check imu observibility   检测IMU是否充分运动  
    {
    ...
    // global sfm  保存到SFM对象中  全局SFM
    // global sfm
    Quaterniond Q[frame_count + 1];
    Vector3d T[frame_count + 1];
    map<int, Vector3d> sfm_tracked_points;
    vector<SFMFeature> sfm_f;
    //这个保存的是所有窗口内看到的feature
    for (auto &it_per_id : f_manager.feature)
    {
        //起始的第一帧
        int imu_j = it_per_id.start_frame - 1;
         //SFMFeature主要记录的是每个路标点由多个连续的图像观测到  特征点信息
    ...
    //ICP恢复位姿
    //保证具有足够的视差,由F矩阵恢复Rt   当前帧既是最后一帧  使用PNP恢复
    //第l帧是从第一帧开始到滑动窗口中第一个满足与  当前帧  的平均视差足够大的帧，会作为参考帧到下面的全局sfm使用
    ...
    //对于所有的图像帧，包括不在滑动窗口中的，提供初始的RT估计，然后solvePnP进行优化,得到每一帧的姿态   所有图像帧进行估计
    //solve pnp for all frame
    map<double, ImageFrame>::iterator frame_it;
    map<int, Vector3d>::iterator it;

    frame_it = all_image_frame.begin( );
    //pnp所有的帧来实现   包括关键帧以及非关键帧 上述是求关键帧的，下面pnp所有的非关键帧
    for (int i = 0; frame_it != all_image_frame.end( ); frame_it++)
    {
        // provide initial guess  如果时间戳能对上
        ...
        cv::cv2eigen(t, T_pnp);
        T_pnp = R_pnp * (-T_pnp);
        //这里就应该是全部变换成以第一帧为参考系的位姿
        frame_it->second.R = R_pnp * RIC[0].transpose();
        frame_it->second.T = T_pnp;
    }

    // Rs Ps ric init  进行视觉惯性初始化  深度加入  并没有优化尺度信息
 ...
 *              得到尺度s和重力g的方向后，需更新所有图像帧在世界坐标系下的Ps、Rs、Vs（已有尺度信息的情况下来判定）
 * @return  bool true：成功
 */
bool Estimator::visualInitialAlignWithDepth()
{
    ...
    VectorXd x;
    //有深度信息进行优化尺度
    //solve scale  解决尺度问题  有深度信息  计算陀螺仪的偏值、重力加速度和速度（包括细化重力）
    bool result = VisualIMUAlignment(all_image_frame, Bgs, g, x);   //进行尺度校准  ？
    if(!result)
    {
    ...
    // change state    得到所有图像帧的位姿，并且将其设为关键帧  
    for (int i = 0; i <= frame_count; i++)
    {
        Matrix3d Ri = all_image_frame[Headers[i].stamp.toSec()].R;
        Vector3d Pi = all_image_frame[Headers[i].stamp.toSec()].T;
    ...
    //triangulat on cam pose , no tic   设定旋转  
    Vector3d TIC_TMP[NUM_OF_CAM];
    for(int i = 0; i < NUM_OF_CAM; i++)
        TIC_TMP[i].setZero();
    ric[0] = RIC[0];
...
//    f_manager.triangulate(Ps, &(TIC_TMP[0]), &(RIC[0]));
    ...
    // do repropagate here  改变陀螺的偏值  重新预积分
    for (int i = 0; i <= WINDOW_SIZE; i++)
    {
        pre_integrations[i]->repropagate(Vector3d::Zero(), Bgs[i]);
    }
    ...
    //不用进行深度估计
    //将重力旋转到z轴上，得到世界坐标系与相机坐标系之间的旋转矩阵
    Matrix3d R0 = Utility::g2R(g); 
    double yaw = Utility::R2ypr(R0 * Rs[0]).x();
    R0 = Utility::ypr2R(Eigen::Vector3d{-yaw, 0, 0}) * R0;
    g = R0 * g;
    //Matrix3d rot_diff = R0 * Rs[0].transpose();
    //所有变量从参考坐标系c0旋转世界坐标系w
    Matrix3d rot_diff = R0;
    for (int i = 0; i <= frame_count; i++)
    {
...
*/
//选择滑动窗口中与最后一帧有足够特征点和视差的某一帧，然后利用五点帧恢复相对旋转和平移   得到R T  并返回bool
...
//            if(average_parallax * 460 > 15 && m_estimator.solveRelativeRT(corres, relative_R, relative_T)) // 30
            if(average_parallax * 460 > 30 && m_estimator.solveRelativeRT_ICP(corres, relative_R, relative_T))
            {
                l = i;
...
void Estimator::solveOdometry()
{
    ...
    if (solver_flag == NON_LINEAR)
    {
        ...
        onlyLineOpt();   // 三角化以后，优化一把   只有线特征的优化
...
void Estimator::optimizationwithLine()
{
    ...
    //end debud
#endif
    // 滑动窗口marg以后，上一次的prior factor   上一次的先验残差
