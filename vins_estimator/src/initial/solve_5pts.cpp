...
//待定
//使用ICP迭代计算    SVD分解
bool MotionEstimator::solveRelativeRT_ICP (vector<pair<Vector3d, Vector3d>> &corres, Matrix3d &R, Vector3d &T)
{
    vector<cv::Point3f> lll;
    vector<cv::Point3f> rrr;
    for (int i = 0;  i< int(corres.size()); i++)
    {
        //当求解深度值大于0时
        if (corres[i].first(2) >0 && corres[i].second(2) >0 ) {
            lll.push_back(cv::Point3f(corres[i].first(0), corres[i].first(1), corres[i].first(2)));
            rrr.push_back(cv::Point3f(corres[i].second(0),corres[i].second(1),corres[i].second(2)));
        }
    }
    //定义中心点
    cv::Point3f p1,p2;
    ...
    Eigen::Matrix3d Roa = U * (V.transpose());

    Eigen::Vector3d Tra = Eigen::Vector3d(p1.x,p1.y,p1.z) - Roa*Eigen::Vector3d(p2.x,p2.y,p2.z);
