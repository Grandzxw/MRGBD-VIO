...
/**
 * @brief   计算尺度，重力加速度和速度
 * @optional    速度、重力向量和尺度初始化Paper -> V-B-2  位姿与速度对齐
 *              相邻帧之间的位置和速度与IMU预积分出来的位置和速度对齐，求解最小二乘
 *              重力细化 -> Paper V-B-3    
 * @param[in]   all_image_frame 所有图像帧构成的map,图像帧保存了位姿，预积分量和关于角点的信息
 * @param[out]  g 重力加速度
 * @param[out]  x 待优化变量，窗口中每帧的速度V[0:n]、重力g  无尺度因子
 * @return      void
*/
//对齐SFM的旋转与位移与IMU计算出的进行对齐     通过滑动窗口中每两帧之间的位置和速度与IMU预积分出来的位置和速度组成最小二乘形式  没有纠正尺度信息
bool LinearAlignmentWithDepth(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    //重力方向与世界坐标的Z轴对齐 
    int all_frame_count = all_image_frame.size();
    int n_state = all_frame_count * 3 + 3;//no scale now  目前无尺度信息
    ...
    map<double, ImageFrame>::iterator frame_i;
    map<double, ImageFrame>::iterator frame_j;
    //对速度重力和尺度  和IMU预积分出来的相对位移相同
    int i = 0;
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
    {
        frame_j = next(frame_i);

        MatrixXd tmp_A(6, 9);//no scale now  无尺度信息
        tmp_A.setZero();
        VectorXd tmp_b(6);
        tmp_b.setZero();

        double dt = frame_j->second.pre_integration->sum_dt;  //总时间
