...
*/
//                                                                   featureId      cameraId                        point                  featureId      cameraId    line
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 4, 1>>>> &image, const map<int, vector<pair<int, Vector4d>>> &lines)
{
    ...
    {
        //初始化每帧
        FeaturePerFrame f_per_fra(id_pts.second[0].second);  //得到点以及深度值
        //初始化id  点特征的id
        ...
        {
            //加深度值   frame_count为起始帧    添加这个图像和特征值信息
            feature.push_back(FeaturePerId(feature_id, frame_count,id_pts.second[0].second[3]));
    ...
    {
        lineFeaturePerFrame f_per_fra(id_line.second[0].second);  // 观测 值
...
void FeatureManager::triangulateWithdepth(Vector3d Ps[], Vector3d tic[], Matrix3d ric[])
{
    for (auto &it_per_id : feature)
    {
        ...
        {
            //每个特征点由多个图像帧看到，这个it_per_frame是这一个特征点在不同图像帧下的特征
...
//        }
//        cout << endl;
        it_per_id.estimated_depth = depth_ave;   //平均的深度值
        it_per_id.estimate_flag = 1;   //通过深度图初始化
